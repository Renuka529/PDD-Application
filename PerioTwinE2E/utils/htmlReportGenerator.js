...
function generateHtmlReport(tests, stats, outputDir) {
  ...
  // Group by category for rendering breakdown
  const categoryStats = {};
  tests.forEach(t => {
...
<head>
  ...
  <title>PerioTwin E2E Automation Report</title>
  <style>
    :root {
      --bg-dark: #0f172a;
      --bg-panel: #1e293b;
    ...
    * {
    ...
    .stat-card.stat-total::after { background-color: var(--accent); }
    ...
    .badge-type { background-color: rgba(59, 130, 246, 0.1); color: var(--accent); }
    .badge-warn { background-color: rgba(245, 158, 11, 0.15); color: var(--warning); }
  ...
  <script>
    ...
    function filterStatus(status, btn) {
      ...
      rows.forEach(row => {
        // Find status column cell
        const statusBadge = row.querySelector('.badge:not(.badge-type)');
...
<body>
  ...
  <header>
    <div class="brand">
      <div>
        <div class="brand-logo">PerioTwin™ E2E Suite</div>
        <div class="brand-tagline">Selenium Automated Test Execution Report</div>
      </div>
    </div>
    <div class="timestamp">
  ...
  <section class="stats-grid">
    ...
    <div class="stat-card">
      <div class="stat-val" style="color: var(--accent);">${avgDuration} ms</div>
    ...
    <section class="results-panel">
      ...
      <div class="table-container">
        <div class="filter-bar">
          ...
          <button class="filter-btn" onclick="filterStatus('Pass', this)" style="border-color: rgba(16, 185, 129, 0.3)">Passed</button>
          <button class="filter-btn" onclick="filterStatus('Fail', this)" style="border-color: rgba(239, 68, 68, 0.3)">Failed</button>
          <button class="filter-btn" onclick="filterStatus('Pending', this)" style="border-color: rgba(245, 158, 11, 0.3)">Pending</button>
        </div>
        ...
        <table>
          <thead>
            <tr>
              ...
              <th style="width: 40%;">Test Case Description</th>
